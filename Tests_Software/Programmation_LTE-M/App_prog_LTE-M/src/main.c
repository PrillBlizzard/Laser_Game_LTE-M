...
// HTTP payload callback function
...
static int run_queries(void)
{
	...
	if (sock4 < 0)
	{
		LOG_ERR("Cannot create HTTP connection.(for GET)");
		return -ECONNABORTED;
	}

	// initialize send HTTP GET request over IPv4
	...
	// Create a socket connection to the server again
	if (IS_ENABLED(CONFIG_NET_IPV4))
	{
		(void)connect_socket(AF_INET, "51.38.237.79", port,
							 &sock4, (struct sockaddr *)&addr4,
							 sizeof(addr4));
	}
...
int main(void)
{
	...
	// start tryning to send queries to the server
